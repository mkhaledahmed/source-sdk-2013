...
ConVar mat_grain_scale_override_nv("mat_grain_scale_override_nv", "4.2");//was 4.2
ConVar mat_nightvision_shadowlift("mat_nightvision_shadowlift", "0.10"); // NEW
...
SHADER_DRAW
{
	...
	DYNAMIC_STATE
	{
		...
		if (mat_grain_scale_override_nv.GetFloat() != -1.0f)
			flNoiseScale = mat_grain_scale_override_nv.GetFloat();
		...
		//========================================
		// NEW: Shadow lift / brightness boost
		//========================================
		float flShadowLift = mat_nightvision_shadowlift.GetFloat();
		pShaderAPI->SetPixelShaderConstant(5, &flShadowLift);
	}
	Draw();
}
END_SHADER