...
ConVar  sk_bullsquid_spit_speed("sk_bullsquid_spit_speed", "0", FCVAR_NONE, "Speed at which an antlion spit grenade travels.");
...
void CNPC_Bullsquid::Precache()
{
	...
	//m_nSquidSpitSprite = PrecacheModel("sprites/greenspit1.vmt");// client side spittle. Removed in favour of the Antlion Worker's Spit.
	...
	PrecacheScriptSound("NPC_Bullsquid.AcidSpit");
...
Vector VecCheckThrowToleranceBullsquid(CBaseEntity *pEdict, const Vector &vecSpot1, Vector vecSpot2, float flSpeed, float flTolerance)
{
...
// Borrowing from npc_antlion.cpp.
//-----------------------------------------------------------------------------
bool CNPC_Bullsquid::GetSpitVector(const Vector &vecStartPos, const Vector &vecTarget, Vector *vecOut)
{
	...
	Vector vecToss = VecCheckThrowToleranceBullsquid(this, vecStartPos, vecTarget, sk_bullsquid_spit_speed.GetFloat(), (10.0f*12.0f));

	// If this failed then try a little faster (flattens the arc)
	if (vecToss == vec3_origin)
	{
		vecToss = VecCheckThrowToleranceBullsquid(this, vecStartPos, vecTarget, sk_bullsquid_spit_speed.GetFloat() * 1.5f, (10.0f*12.0f));
		if (vecToss == vec3_origin)
			return false;
	}
