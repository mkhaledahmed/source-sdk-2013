...
public:
	...
	virtual void SetObjectCollisionBox(void)
	{
		...
		// (SetAbsMins/SetAbsMaxs don't exist in this SDK -- SetCollisionBounds()
		// takes both at once, and takes them relative to the entity's own
		// origin rather than GetAbsOrigin()-offset absolute positions.)
		SetCollisionBounds( Vector(-64.f, -64.f, 0.f), Vector(64.f, 64.f, 80.f) );
	}
...
// NOTE: the model's QC has "// ACT_CHARGE_LOOP 1" commented out directly
// above "ACT_RUN 1" in the chargeloop sequence block -- the model was
// switched to use the stock ACT_RUN for its charge/run cycle, so that's
// what StartTask()/TASK_GET_CHARGE_PATH requests now. There's no
// "ACT_CHARGE_LOOP" in the compiled model at all.
...
int CCrabSynth::OnTakeDamage_Alive(const CTakeDamageInfo& inputInfo)
{
	...
	if (info.GetDamageType() & DMG_BLAST)
	{
		if (info.GetInflictor())
		{
			...
			if (info.GetInflictor()->GetAbsOrigin().z - GetAbsOrigin().z < ((WorldAlignMaxs().z - WorldAlignMins().z) * 0.5f))
			{
