...
{
	CPASAttenuationFilter filter(this);
	EmitSound(filter, entindex(), "NPC_Houndeye.WarmUp");
}
...
{
	CPASAttenuationFilter filter(this);
	EmitSound(filter, entindex(), "NPC_Houndeye.Hunt");
}
...
void CNPC_Houndeye::AlertSound(void)
{
	...
	CPASAttenuationFilter filter(this);
	EmitSound(filter, entindex(), "NPC_Houndeye.Alert");
}
...
{
	CPASAttenuationFilter filter(this);
	EmitSound(filter, entindex(), "NPC_Houndeye.Die");
}
...
{
	CPASAttenuationFilter filter(this);
	EmitSound(filter, entindex(), "NPC_Houndeye.Pain");
}
...
void CNPC_Houndeye::HandleAnimEvent(animevent_t *pEvent)
{
	...
	{
		CPASAttenuationFilter filter(this);
		EmitSound(filter, entindex(), "NPC_Houndeye.Anger1");
	}
	break;
	...
	case HOUND_AE_ANGERSOUND2:
	{
		...
		EmitSound(filter2, entindex(), "NPC_Houndeye.Anger2");
	}
	break;
...
int CNPC_Houndeye::SquadRecruit(int searchRadius, int maxMembers)
{
	...
	if (m_SquadName != NULL_STRING)
	{
		...
		pEntity = gEntList.FindEntityByClassname(pEntity, "npc_houndeye");
		...
		while ((pEntity = gEntList.FindEntityInSphere(pEntity, GetAbsOrigin(), searchRadius)) != NULL && squadCount < maxMembers)
		{
			...
			if (pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_hCine)
			{
				...
				if (!pRecruit->m_pSquad && pRecruit->Classify() == iMyClass &&
					((iMyClass != CLASS_HOUNDEYE) || FClassnameIs(this, pRecruit->GetClassname())) &&
