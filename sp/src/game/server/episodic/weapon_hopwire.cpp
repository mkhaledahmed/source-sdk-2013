...
//-----------------------------------------------------------------------------
class CWeaponHopwire : public CBaseHLCombatWeapon, public IQuickThrowable
{
...
public:
	...
	// IQuickThrowable -- lets the Q/E quick-throw system fire this weapon's
	// throw while a completely different weapon is active.
	virtual bool QuickThrow(CBasePlayer* pPlayer);
...
void CWeaponHopwire::Precache(void)
{
	...
	PrecacheScriptSound("WeaponFrag.Lob");
...
void CWeaponHopwire::SecondaryAttack(void)
{
	...
	// See if we're in trap mode -- same early-detonate behavior PrimaryAttack()
	// has, kept consistent between the two attacks.
...
//-----------------------------------------------------------------------------
// Purpose: IQuickThrowable -- instant throw for the Q/E quick-throw system.
// Always throws a fresh hopwire (skips the trap-mode "spring the existing
// one instead" branch PrimaryAttack() has -- quick-throw is meant to always
// throw, not detonate an already-placed trap).
//-----------------------------------------------------------------------------
bool CWeaponHopwire::QuickThrow(CBasePlayer* pPlayer)
{
	if (!pPlayer)
		return false;
	...
	if (m_fDrawbackFinished)
	{
		...
		if (pOwner)
		{
			...
			case GRENADE_PAUSED_SECONDARY:
				...
				{
					// Matches weapon_frag.cpp's real secondary-attack behavior:
					// crouching lobs, standing rolls it along the ground.
...
void CWeaponHopwire::LobGrenade(CBasePlayer* pPlayer)
{
	...
	// Named sound, matching the Throw/Roll convention, instead of the
	// generic WPN_DOUBLE category.
	EmitSound("WeaponFrag.Lob");
