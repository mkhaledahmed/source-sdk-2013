...
//-----------------------------------------------------------------------------
class CWeaponFrag : public CBaseHLCombatWeapon, public IQuickThrowable
{
...
// Deliberately skips the normal wind-up/pause/release state machine
// (m_AttackPaused, ACT_VM_PULLBACK_HIGH, etc.) since none of that applies
// when this weapon isn't the one currently in the player's hands.
//-----------------------------------------------------------------------------
bool CWeaponFrag::QuickThrow(CBasePlayer* pPlayer)
{
	if (!pPlayer)
		return false;
...
void CWeaponFrag::LobGrenade(CBasePlayer* pPlayer)
{
	...
	// generic WPN_DOUBLE category (which had no confirmed soundscript entry).
