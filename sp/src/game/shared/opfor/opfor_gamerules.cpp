...
REGISTER_GAMERULES_CLASS( COpposingForce );

BEGIN_NETWORK_TABLE_NOBASE( COpposingForce, DT_OPFORGameRules )
	#ifdef CLIENT_DLL
...
BEGIN_DATADESC( COpposingForceProxy )
...
LINK_ENTITY_TO_CLASS( opfor_gamerules, COpposingForceProxy );
IMPLEMENT_NETWORKCLASS_ALIASED( OpposingForceProxy, DT_OpposingForceProxy )

#if defined(MAPBASE) && defined(GAME_DLL)
void COpposingForceProxy::InputEpisodicOn( inputdata_t &inputdata ) { KeyValue("SetEpisodic", "1"); }
void COpposingForceProxy::InputEpisodicOff( inputdata_t &inputdata ) { KeyValue("SetEpisodic", "0"); }
void COpposingForceProxy::InputSetFriendlyFire( inputdata_t &inputdata ) { KeyValue("GlobalFriendlyFire", inputdata.value.String()); }
void COpposingForceProxy::InputSetDefaultCitizenType( inputdata_t &inputdata ) { KeyValue("DefaultCitizenType", inputdata.value.String()); }
void COpposingForceProxy::InputSetLegacyFlashlight( inputdata_t &inputdata ) { KeyValue("SetLegacyFlashlight", inputdata.value.String()); }
void COpposingForceProxy::InputSetPlayerSquadAutosummon( inputdata_t &inputdata ) { KeyValue("SetPlayerSquadAutosummon", inputdata.value.String()); }
void COpposingForceProxy::InputSetStunstickPickupBehavior( inputdata_t &inputdata ) { KeyValue("SetStunstickPickupBehavior", inputdata.value.String()); }
void COpposingForceProxy::InputSetAllowSPRespawn( inputdata_t &inputdata ) { KeyValue( "SetAllowSPRespawn", inputdata.value.String() ); }
...
//-----------------------------------------------------------------------------
bool COpposingForceProxy::KeyValue( const char *szKeyName, const char *szValue )
{
	if (FStrEq(szKeyName, "DefaultCitizenType"))
	{
		OPFORGameRules()->SetDefaultCitizenType(atoi(szValue));
	}
	else if (FStrEq(szKeyName, "GlobalFriendlyFire"))
	{
		OPFORGameRules()->SetGlobalFriendlyFire(TO_THREESTATE(atoi(szValue)));
	}
	...
	{
		OPFORGameRules()->SetAutosummonDisabled(FStrEq(szValue, "0"));
	}
	else if (FStrEq(szKeyName, "SetStunstickPickupBehavior"))
	{
		OPFORGameRules()->SetStunstickPickupBehavior(atoi(szValue));
	}
	else if (FStrEq(szKeyName, "SetAllowSPRespawn"))
	{
		OPFORGameRules()->SetAllowSPRespawn(!FStrEq(szValue, "0"));
	}
	else
	{
...
bool COpposingForceProxy::GetKeyValue( const char *szKeyName, char *szValue, int iMaxLen )
{
	if (FStrEq(szKeyName, "DefaultCitizenType"))
	{
		Q_snprintf( szValue, iMaxLen, "%i", OPFORGameRules()->GetDefaultCitizenType() );
	}
	else if (FStrEq(szKeyName, "GlobalFriendlyFire"))
	{
		Q_snprintf( szValue, iMaxLen, "%i", OPFORGameRules()->GlobalFriendlyFire() );
	}
	else if (FStrEq(szKeyName, "SetEpisodic") && !FStrEq(szValue, "2"))
	{
	...
	{
		Q_snprintf( szValue, iMaxLen, "%d", OPFORGameRules()->AutosummonDisabled() );
	}
	else if (FStrEq(szKeyName, "SetStunstickPickupBehavior"))
	{
		Q_snprintf( szValue, iMaxLen, "%i", OPFORGameRules()->GetStunstickPickupBehavior() );
	}
	else if (FStrEq(szKeyName, "SetAllowSPRespawn"))
	{
		Q_snprintf( szValue, iMaxLen, "%d", OPFORGameRules()->AllowSPRespawn() );
	}
	else
	{
...
//-----------------------------------------------------------------------------
int COpposingForceProxy::Save( ISave &save )
{
	m_save_DefaultCitizenType = OPFORGameRules()->GetDefaultCitizenType();
	m_save_PlayerSquadAutosummonDisabled = OPFORGameRules()->AutosummonDisabled();
	...
	m_save_StunstickPickupBehavior = OPFORGameRules()->GetStunstickPickupBehavior();

	m_save_AllowSPRespawn = OPFORGameRules()->AllowSPRespawn();
...
//-----------------------------------------------------------------------------
int COpposingForceProxy::Restore( IRestore &restore )
{
	int base = BaseClass::Restore(restore);

	OPFORGameRules()->SetDefaultCitizenType(m_save_DefaultCitizenType);
	OPFORGameRules()->SetAutosummonDisabled(m_save_PlayerSquadAutosummonDisabled);
	...
	OPFORGameRules()->SetStunstickPickupBehavior(m_save_StunstickPickupBehavior);

	OPFORGameRules()->SetAllowSPRespawn(m_save_AllowSPRespawn);
...
// Purpose: 
//-----------------------------------------------------------------------------
void COpposingForceProxy::UpdateOnRemove()
{
...
#ifdef CLIENT_DLL
	void RecvProxy_OPFORGameRules( const RecvProp *pProp, void **pOut, void *pData, int objectID )
	{
		COpposingForce *pRules = OPFORGameRules();
		Assert( pRules );
		*pOut = pRules;
	}

	BEGIN_RECV_TABLE( COpposingForceProxy, DT_OpposingForceProxy )
		RecvPropDataTable( "opfor_gamerules_data", 0, 0, &REFERENCE_RECV_TABLE( DT_OPFORGameRules ), RecvProxy_OPFORGameRules )
	END_RECV_TABLE()
#else
	void* SendProxy_OPFORGameRules( const SendProp *pProp, const void *pStructBase, const void *pData, CSendProxyRecipients *pRecipients, int objectID )
	{
		COpposingForce *pRules = OPFORGameRules();
		Assert( pRules );
	...
	BEGIN_SEND_TABLE( COpposingForceProxy, DT_OpposingForceProxy )
		SendPropDataTable( "opfor_gamerules_data", 0, &REFERENCE_SEND_TABLE( DT_OPFORGameRules ), SendProxy_OPFORGameRules )
	END_SEND_TABLE()
#endif
...
//-----------------------------------------------------------------------------
int COpposingForce::Damage_GetTimeBased( void )
{
#ifdef HL2_EPISODIC
...
// Output :		bool
//-----------------------------------------------------------------------------
bool COpposingForce::Damage_IsTimeBased( int iDmgType )
{
...
ConVar  alyx_darkness_force( "alyx_darkness_force", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
	...
	//-----------------------------------------------------------------------------
	bool COpposingForce::ClientCommand( CBaseEntity *pEdict, const CCommand &args )
	{
	...
	//-----------------------------------------------------------------------------
	void COpposingForce::PlayerSpawn( CBasePlayer *pPlayer )
	{
	}
	...
	// Output  :
	//------------------------------------------------------------------------------
	void COpposingForce::InitDefaultAIRelationships( void )
	{
		int i, j;
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_ANTLION,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_BARNACLE,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_BULLSEYE,			CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_BULLSQUID,			CLASS_CRAB_SYNTH,			D_HT, 1);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_CITIZEN_PASSIVE,	CLASS_CRAB_SYNTH,			D_FR, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_CITIZEN_REBEL,		CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE_GUNSHIP,		CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE_HUNTER,		CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_CONSCRIPT,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_FLARE,			CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_HOUNDEYE,			CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_MANHACK,			CLASS_CRAB_SYNTH,			D_HT,-1);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_METROPOLICE,		CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_MILITARY,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_MISSILE,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_NONE,				CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_PLAYER,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_PLAYER_ALLY,			CLASS_CRAB_SYNTH,			D_FR, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_PLAYER_ALLY_VITAL,	CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_SCANNER,			CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_STALKER,			CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_VORTIGAUNT,		CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_ZOMBIE,			CLASS_CRAB_SYNTH,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_PROTOSNIPER,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_EARTH_FAUNA,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_HACKED_ROLLERMINE,			CLASS_CRAB_SYNTH,			D_HT, 0);
		...
		////CBaseCombatCharacter::SetDefaultRelationship(CLASS_CRAB_SYNTH,			CLASS_BULLSQUID,		D_FR, 0);
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_CRAB_SYNTH, CLASS_CITIZEN_PASSIVE, D_HT, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_CRAB_SYNTH, CLASS_CRAB_SYNTH, D_NU, 0);
		////CBaseCombatCharacter::SetDefaultRelationship(CLASS_CRAB_SYNTH,			CLASS_HOUNDEYE,			D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_CRAB_SYNTH, CLASS_ZOMBIE, D_NU, 0);
		...
		//CBaseCombatCharacter::SetDefaultRelationship(CLASS_CRAB_SYNTH, CLASS_HACKED_ROLLERMINE, D_FR, 0);
	}
	...
	// Output  :
	//------------------------------------------------------------------------------
	const char* COpposingForce::AIClassText(int classType)
	{
		switch (classType)
		{
	...
	void COpposingForce::PlayerThink( CBasePlayer *pPlayer )
	{
	}

	void COpposingForce::Think( void )
	{
		BaseClass::Think();
	...
	//-----------------------------------------------------------------------------
	float COpposingForce::GetAmmoDamage( CBaseEntity *pAttacker, CBaseEntity *pVictim, int nAmmoType )
	{
		float flDamage = 0.0f;
   	...
   	//-----------------------------------------------------------------------------
  	//-----------------------------------------------------------------------------
 	bool COpposingForce::AllowDamage( CBaseEntity *pVictim, const CTakeDamageInfo &info )
  	{
#ifndef CLIENT_DLL
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool COpposingForce::NPC_ShouldDropHealth( CBasePlayer *pRecipient )
	{
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool COpposingForce::NPC_ShouldDropGrenade( CBasePlayer *pRecipient )
	{
		// Can only do this every so often
	...
	//-----------------------------------------------------------------------------
	void COpposingForce::NPC_DroppedHealth( void )
	{
	...
	//-----------------------------------------------------------------------------
	void COpposingForce::NPC_DroppedGrenade( void )
	{
...
// ------------------------------------------------------------------------------------ //
// Shared COpposingForce implementation.
// ------------------------------------------------------------------------------------ //
bool COpposingForce::ShouldCollide( int collisionGroup0, int collisionGroup1 )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void COpposingForce::AdjustPlayerDamageTaken( CTakeDamageInfo *pInfo )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
float COpposingForce::AdjustPlayerDamageInflicted( float damage )
{
	switch( GetSkillLevel() ) 
	{
	case SKILL_EASY:
...
//---------------------------------------------------------
//---------------------------------------------------------
bool COpposingForce::ShouldUseRobustRadiusDamage(CBaseEntity *pEntity)
{
#ifdef CLIENT_DLL
	return false;
#endif
...
//---------------------------------------------------------
//---------------------------------------------------------
bool COpposingForce::ShouldAutoAim( CBasePlayer *pPlayer, edict_t *target )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
float COpposingForce::GetAutoAimScale( CBasePlayer *pPlayer )
{
#ifdef _X360
	return 1.0f;
#else
...
//---------------------------------------------------------
//---------------------------------------------------------
float COpposingForce::GetAmmoQuantityScale( int iAmmoIndex )
{
	switch( GetSkillLevel() )
	{
	case SKILL_EASY:
...
void COpposingForce::LevelInitPreEntity()
{
...
//-----------------------------------------------------------------------------
bool COpposingForce::IsAlyxInDarknessMode()
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
bool COpposingForce::ShouldBurningPropsEmitLight()
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
int COpposingForce::GetDefaultCitizenType()
{
...
//-----------------------------------------------------------------------------
void COpposingForce::SetDefaultCitizenType(int val)
{
...
//-----------------------------------------------------------------------------
ThreeState_t COpposingForce::GlobalFriendlyFire()
{
...
//-----------------------------------------------------------------------------
void COpposingForce::SetGlobalFriendlyFire(ThreeState_t val)
{
...
//-----------------------------------------------------------------------------
bool COpposingForce::AutosummonDisabled()
{
...
//-----------------------------------------------------------------------------
void COpposingForce::SetAutosummonDisabled(bool toggle)
{
...
//-----------------------------------------------------------------------------
int COpposingForce::GetStunstickPickupBehavior()
{
...
//-----------------------------------------------------------------------------
void COpposingForce::SetStunstickPickupBehavior(int val)
{
...
//-----------------------------------------------------------------------------
bool COpposingForce::AllowSPRespawn()
{
...
//-----------------------------------------------------------------------------
void COpposingForce::SetAllowSPRespawn( bool toggle )
{
	m_bAllowSPRespawn = toggle;
}

//BEGIN_SIMPLE_DATADESC( COpposingForce )
//
