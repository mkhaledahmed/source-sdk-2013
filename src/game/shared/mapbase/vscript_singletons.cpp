...
static void FireGameEvent( const char* szEvent, HSCRIPT hTable )
{
	...
	if ( event )
	{
		...
		while ( ( nIterator = g_pScriptVM->GetKeyValue( hTable, nIterator, &key, &val ) ) != -1 )
		{
			switch ( val.GetType() )
			{
				case FIELD_FLOAT:   event->SetFloat ( key.Get<const char*>(), val.Get<float>()     ); break;
				case FIELD_INTEGER: event->SetInt   ( key.Get<const char*>(), val.Get<int>()       ); break;
				case FIELD_BOOLEAN: event->SetBool  ( key.Get<const char*>(), val.Get<bool>()      ); break;
				case FIELD_CSTRING: event->SetString( key.Get<const char*>(), val.Get<const char*>() ); break;
			}
...
void CScriptSaveRestoreUtil::SaveTable( const char *szId, HSCRIPT hTable )
{
	...
	while ( ( nIterator = g_pScriptVM->GetKeyValue( hTable, nIterator, &key, &val ) ) != -1 )
	{
		switch ( val.GetType() )
		{
			case FIELD_FLOAT:   pKV->SetFloat ( key.Get<const char*>(), val.Get<float>()     ); break;
			case FIELD_INTEGER: pKV->SetInt   ( key.Get<const char*>(), val.Get<int>()       ); break;
			case FIELD_BOOLEAN: pKV->SetBool  ( key.Get<const char*>(), val.Get<bool>()      ); break;
			case FIELD_CSTRING: pKV->SetString( key.Get<const char*>(), val.Get<const char*>() ); break;
		}
...
public:
	...
	void CommandCallback( const CCommand &command )
	{
		...
		if ( m_pLinked && (ret.GetType() == FIELD_BOOLEAN) && ret.Get<bool>())
		{
	...
	int CommandCompletionCallback( const char *partial, CUtlVector< CUtlString > &commands )
	{
		...
		{
			if ( val.GetType() == FIELD_CSTRING)
			{
				CUtlString &s = commands.Element( commands.AddToTail() );
				int len = V_strlen( val.Get<const char*>() );

				if ( len <= COMMAND_COMPLETION_ITEM_LENGTH - 1 )
				{
					s.Set( val.Get<const char*>() );
				}
				else
				{
					s.SetDirect( val.Get<const char*>(), COMMAND_COMPLETION_ITEM_LENGTH - 1 );
				}
