...
void ParseScriptTableKeyValues( CBaseEntity *pEntity, HSCRIPT hKV )
{
	...
	while ((nIterator = g_pScriptVM->GetKeyValue( hKV, nIterator, &varKey, &varValue )) != -1)
	{
		...
		{
			case FIELD_CSTRING:		pEntity->KeyValue( varKey.Get<const char*>(), varValue.Get<const char*>()); break;
			case FIELD_INTEGER:		pEntity->KeyValueFromInt( varKey.Get<const char*>(), varValue.Get<int>() ); break;
			case FIELD_FLOAT:		pEntity->KeyValue( varKey.Get<const char*>(), varValue.Get<float>() ); break;
			case FIELD_VECTOR:		pEntity->KeyValue( varKey.Get<const char*>(), varValue.Get<Vector>() ); break;
			case FIELD_HSCRIPT:
			{
				if ( varValue.Get<HSCRIPT>() )
				{
					// Entity
					if (ToEnt( varValue.Get<HSCRIPT>() ))
					{
						pEntity->KeyValue( varKey.Get<const char*>(), STRING( ToEnt( varValue.Get<HSCRIPT>() )->GetEntityName() ) );
					}

					// Color
					else if (Color *color = HScriptToClass<Color>( varValue.Get<HSCRIPT>() ))
					{
						char szTemp[64];
						Q_snprintf( szTemp, sizeof( szTemp ), "%i %i %i %i", color->r(), color->g(), color->b(), color->a() );
						pEntity->KeyValue( varKey.Get<const char*>(), szTemp );
					}
				}
				break;
			}
